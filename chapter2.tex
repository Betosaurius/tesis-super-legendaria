...
In \todo{agregar cita}, the monopolarity problem for cographs was solved. We
could save some work by considering only those chordal graphs that are not
cographs, that is, those that contain an induced $P_4$. Before stating the main
result of this section, we prove the following properties regarding the
structure of the exact neighborhoods of a chordal graph with an induces $P_4$
and no induced $P_5$ (notice that the house contains a $C_4$, so no chordal
graph contains a house).
...
\begin{lemma}
    ...
    \begin{enumerate}[(i)]
        \item If $X\subseteq S$ is such that it contains $x$ and $y$
        non-consecutive vertices of $P$ and it does not contain any vertex of
        $P$ between $x$ and $y$, then $N_X=\varnothing$.
        \item $N_a = N_d = \varnothing$.
        \item If $\varnothing \subsetneq X \subsetneq S$, then $N_X$ is
        completely not adjacent to $N_{S\setminus X}$.
        \item If $X,Y\subseteq S$ are such that their intersection has two
        non-consecutive vertices of $P$, then $N_X$ is completely adjacent to
        $N_Y$. In particular, this implies that $N_X$ is a clique.
        \item If $X$ consists of three consecutive vertices of $P$ and $Y$
        intersects $X$ in two consecutive vertices of $P$, one being an endpoint
        of $P$, then $N_X$ is completely adjacent to $N_Y$.
        \item If $\varnothing\subsetneq X\subset S$ of cardinality at most two,
        then $N_{\varnothing}$ is completely not adjacent to $N_X$, unless
        $X=\l{bc}$. Moreover, each connected component of $G[N_\varnothing]$ is
        homogeneous to vertices in $N_Y$ for $Y\subseteq S$ of cardinality at
        exactly three, and to $N_{bc}$, and each component is adjacent to at
        least one vertex of the prior sets or to $N_S$.
    \end{enumerate}
\end{lemma}
...
\begin{proof}
    ...
    \begin{enumerate}[(i)]
        ...
        non-consecutive vertices of $P$ in $X$, either one or two vertices of
        $P$ are between $x$ and $z$. If there is just one, $y$, then
        $\p{w,x,y,z,w}$ is a $C_4$, contradicting that $G$ is chordal. If there
        are two, $y$ and $y'$, then $\p{w,x,y,y',z,w}$ is a $C_5$, contradicting
        that $G$ is chordal.
        ...
        $N_{S\setminus X}$ such that $w$ and $z$ are adjacent. Since $X$ is not
        empty nor $S$, there are two consecutive vertices of $P$, say $x$ and
        $y$, such that $x\in X$ and $y\in S\setminus X$. Then $\p{w,x,y,z,w}$ is
        a $C_4$.
        \item Suppose $w$ is a vertex in $N_X$ and $z$ is a vertex in $N_Y$ such
        that $w$ and $z$ are not adjacent. Since $X\cap Y$ contains two
        non-consecutive vertices of $P$, say $x$ and $y$, then $\p{w,x,z,y,w}$
        is a $C_4$.
        \item Suppose without loss of generality that $X=\l{a,b,c}$, so
        $Y=\l{a,b}$. If $w$ and $w'$ are vertices in $N_X$ and $N_Y$,
        respectively, such that $w$ and $w'$ are not adjacent, then
        $\p{d,c,w,a,w'}$ is a $P_5$.
        \item Since $X$ is not $\l{bc}$ and it has at most two vertices, there
        are two consecutive vertices of $P$ that are not in $X$, say $x$ and
        $y$, such that $y$ is adjacent to some vertex $z$ in $X$. If $w$ and
        $w'$ are vertices in $N_\varnothing$ and $N_X$, respectively, such that
        $w$ and $w'$ are adjacent, then $\p{w,w',z,y,x}$ is a $P_5$. The
        remaining part of the statement follows from the fact that $G$ is
        connected and, if $x$ and $y$ are adjacent in $N_\varnothing$ and such
        that $x$ is adjacent to $w$ in $N_Y$, but $y$ is not adjacent to $w$,
        then $\p{y,x,w,u,v}$ is a $P_5$, where $u$ and $v$ are two vertices in
        $P$ with $u$ in $Y$ and $v$ in $S\setminus Y$.
    \end{enumerate}
\end{proof}
    ...
    \begin{description}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                completely not adjacent to $N_c$.
                
                Now, for each edge $vw$ in $G[N_{cd}\setminus K]$, we claim
                thatat least one of $v$ and $w$ is completely adjacent to $K$.
                If not, there are vertices $v'$ and $w'$ in $K$ such that $v'$
                is not adjacent to $v$ and $w'$ is not adjacent to $w$. Since
                $v'$ is in $K$, there is some vertex $u$ in $N_c$ that is
                adjacent to $v'$. Since $G$ is $H_2$-free and $v$ is not
                adjacent to $v'$, it follows that $v'$ is adjacent to $w$.
                Similarly, $w'$ is adjacent to $v$. Then, $\p{v,w',v',w,v}$ is a
                $C_4$, a contradiction. Hence, vertices of $N_{cd}\setminus K$
                can be partitioned into two sets, $S$ and $K'$, such that $S$ is
                stable and $K'$ are vertices that are completely adjacent to
                $K$, with the additional property that $S$ is maximal, that is,
                every vertex in $K'$ is adjacent to at least one vertex in $S$.
                We claim that $K'$ is a clique. If not, there are two vertices
                that are not adjacent, say $v$ and $w$. By the maximality of
                $S$, there are vertices $v'$ and $w'$ in $S$ such that $v$ is
                adjacent to $v'$ and $w$ is adjacent to $w'$. If $v'\neq w'$,
                ...
                completely adjacent to $N_{cd}$, while (iv) of the same lemma
                implies that $N_{bcd}$ is a clique.
                ...
                Since $G$ is $H_7$-free and $N_b$ is completely not adjacent to
                $N_{cd}$ by (iii) of \cref{lem:neighborhoods-chordal}, it
                follows that $N_b$ is $P_3$-free. Furthermore, since $G$ is
                $H_2$-free, $N_b$ is completely not adjacent to $N_{bcd}$.

                For $N_\varnothing$, by (vi) of
                \cref{lem:neighborhoods-chordal}, each connected component of
                $G[N_\varnothing]$ is completely adjacent to at least one vertex
                of $N_{bcd}$, since this is the only non-empty set to which it
                can be adjacent. Since $G$ is $H_4$-free and $N_{bcd}$ is
                completely adjacent to $N_{cd}$, each component of
                $G[N_\varnothing]$ is trivial, that is, $N_\varnothing$ is an
                independent set.
