...
% Remove section number from theorem-like environments
...
\begin{corollary}
    There is a certifying algorithm, running on time $O(|V|^2)$, that decides
    whether a $(P_5,\text{house})$-free graph admits an $(\infty,1)$-polar
    partition. Additionally, the minimal obstructions to $(\infty,1)$-polarity
    in the class of $(P_5,\text{house})$-free graphs are the graphs
    $\overline{H_i}$ with $i\in \l{1,...,14}$.
\end{corollary}
...
$F$ with at least one nontrivial component, $G$ being $F$-free implies that $G'$
is $F$-free. The same holds for all graphs $F$ with every vertex having at least
...
representative sample not intended to be exhaustive --- the proofs of
\textsc{Monopolarity} being NP-complete for these families are derived from the
results in \cite{Le2014}.
...
\begin{corollary}
    ...
    \begin{enumerate}
        ...
        \item $\mathcal{F} = \l{C_5,\dots,C_n}$ for all $n\geq 5$
        \item $\mathcal{F} = \l{K_1 + K_3, \text{house}}$
        \item $\mathcal{F} = \l{P}$
        \item $\mathcal{F} = \l{\overline{2P_3}}$
        \item $\mathcal{F} = \l{\text{bull},\text{house}}$
        \item $\mathcal{F} = \l{\text{bull},\text{kite}}$
    \end{enumerate}
\end{corollary}