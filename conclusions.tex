...
certyfing algorithms to solve \textsc{Monopolarity} on $(P_5,\text{house})$-free
depending on whether the graph is chordal, or contains an induced copy of either
$C_4$ or $C_5$. Whenever the graphs contains an induced copy of $C_4$ or $C_5$,
the algotithm requires for such a copy to be given as an input. Nonetheless, if
the algorithm changes its state to execute the procedures of either
\Cref{cap:c5} or \Cref{cap:no_c5}, it happens because it has already found the
copy of an induced cycle. This result is summarized in the next corollary as a
...
\begin{corollary}
    There is a linear-time certyfing algorithm that decides if a $C_4$-free
    graph is monopolar and $(P_5,\text{house})$-free.
\end{corollary}

We leave as an open problem to complete the structure of $C_4$-free graphs,
which in turn will generalize the last corollary to graphs that are not
$C_4$-free. Once that is done, we would have that the family of monopolar
$(P_5,\text{house})$-free graphs is linear-time recognizable with a certyfing
algorithm. For the moment, observe that the algorithm still can return a
monopolar partition even when the input graph is not $(P_5,\text{house})$-free.
Whenever the graph is chordal, we have ensured that it also returns an induced
copy of $P_5$ if that is the case, and the same goes for graphs with an induced
copy of $C_5$. Lastly, if the algorithm assigns one of the monopolar partitions
given in the proof of \Cref{thm:monopolar_c4}, then we cannot ensure that the
input graph is indeed $(P_5,\text{house})$-free.
...
\begin{lemma}[\cite{Contreras2024}] If $G$ is a graph, then $G$ is disconnected
minimal polar obstruction if and only if $G \cong P_3 + H$, where $H$ is a
minimal monopolar obstruction which is not a minimal polar obstruction.
\end{lemma}
...
Left is to study the connected $(P_5,\text{house})$-free minimal polar
obstructions. We leave it as an open problem to characterize them, but observe
that it suffices to consider those that also have a connected complement, so all
obstructions left to find are cographs (graphs with no induced $P_4$).

Now we briefley turn out attention to the complementary problem of
...
\begin{corollary}
    There is a $O(|V|^2)$ certyfing algorithm that decides whether a
...
The aforementioned construction is such that preserves the absence of certain
structures within graphs. For instance, consider a graph $F$ with no universal
vertex nor a pair of nonadjacent universal vertices. It is clear that any
$F$-free graph $G$ is such that the $G'$ constructed in the proof of the last
result is also $F$-free. In particular, for any disconnected graph $F$ with at
least one non-trivial component, $G$ being $F$-free implies that $G'$ is
$F$-free. The same holds for all graphs $F$ with every vertex having at least
two non-neighbors, which includes all cycles $C_n$ for $n\geq 5$. This
...
\begin{corollary}
    ...
    \begin{enumerate}
        \item $\mathcal{F} = \l{2K_3,\text{house}}$
        \item $\mathcal{F} = \l{2K_4,\text{house}}$
        \item $\mathcal{F} = \l{C_5,\text{house}}$
        \item $\mathcal{F} = \l{C_6,\text{house}}$
        \item $\mathcal{F} = \l{C_6,\overline{C_6}}$
