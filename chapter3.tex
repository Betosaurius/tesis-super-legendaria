...
at least $4$. Since we only consider $P_5$-free graphs, we know that each
induced cycle has length at most $5$: if when checking chordality, the algorithm
finds an induced cycle of length at least $6$, then is returns $5$ consecutive
vertices of such a cycle as an induced copy of $P_5$. We devote this chapter to
the family of graphs with an induced copy of $C_5$.


\section{Characterization of monopolarity}
\label{sec:c5_characterization}

For all graphs in the present chapter with an induced copy of $C_5$, the
labelling of the vertices of the $5$-cycle is determined by the drawing shown
below. Thus, vertices of a $5$-cycle are labeled from $a$ to $e$, starting with
the rightmost vertex and following a counter-clockwise order (see
\Cref{fig:labels_c5}).
...
Recall that the full algorithm first checks if the input graph is chordal. If it
finds an induced copy of $C_5$, then it executes the processes described in the
following section. Otherwise, if it detects an induced copy of $C_4$, then it
proceeds to execute the instructions that will be described in the next chapter.
If at any point of such instructions the algorithm finds an induced copy of
$C_5$ (that, by chance, was not detected by the algorithm that verifies
...
We now prove that graphs $H_1$ all the way to $H_{13}$ characterize monopolarity
for the family of $(P_5,\text{house})$-free graphs with an induced copy of
$C_5$. To do so, we prove a series of lemmas describing the exact neighborhoods
of a 5-cycle. Throughout the following proofs we always denote an induced copy
of $C_5$ in a graph $G$ by $C = \p{a,b,c,d,e,a}$ and the set $\l{a,b,c,d,e}$ is
denoted by $Z$.
...
the only type of 2-bag that can be nonempty is that consisting of two
...
The final lemma addresses the structure of exact neighborhoods, already
...
graphs that prevent $G$ from being monopolar. Different from the way it was done
in the previous chapter, we have already stated the necessary and sufficient
conditions that must hold within the exact neighborhoods for the graph to also
be $(P_5,\text{house})$-free. Thus, once a monopolar partition for a graphs is
found, we use the previous lemmas to deduce the complete structure of the graph. 
...
\begin{proof}
    ...
    implies that no vertex of $C$ is complete to all the $3$-bags. Hence,
...
\section[Structure of the graph and time complexity of the algorithm]{Structure
of the graph and time complexity of the algorithm %
\sectionmark{Structure and time complexity}}

As stated before the last theorem, we have shown exhaustively how the monopolar
graphs in this family look like. Moreover, up to symmetry, observe that all
monopolar partitions given in the proof of \Cref{thm:monopolar_c5} have the same
structure of \Cref{fig:monopolar_partition_1}. Thus, the theorem yields the
following corollary.
...
\begin{figure}[htb!]
    ...
    \begin{tikzpicture}[every node/.style={vertex}, node
        distance=1cm, on grid, edge]
        \foreach \labeltext in {{$N_{be} \cup \l{a}$}, {$N_{cd} \cup \l{d}$},
        {$N_{abc} \cup \l{b}$}, {$N_{bcd} \cup \l{c}$}, {$N_{ade} \cup \l{e}$},
        {$N_{ad}$}}{
        ...
        \begin{scope}
            \node (A) [fill=none, minimum size=\maxsize] at (18:2.5) {$N_{be} \cup \l{a}$}; 
            \node (B) [fill=none, minimum size=\maxsize] at (90:2.5) {$N_{abc} \cup \l{b}$};
            \node (C) [fill=none, minimum size=\maxsize] at (162:2.5) {$N_{bcd} \cup \l{c}$};
            \node (D) [fill=none, minimum size=\maxsize] at (234:2.5) {$N_{cd} \cup \l{d}$};
            \node (E) [fill=none, minimum size=\maxsize] at (306:2.5) {$N_{ade} \cup \l{e}$};
            \node (AD) [fill=none, minimum size=\maxsize] at (0,0) {$N_{ad}$};
        \end{scope}
        ...
        \draw[double] (AD) to (A);
    \end{tikzpicture}
    \caption{Structure of monopolar $(P_5,\text{house})$-free graphs with an
        induced copy of $C_5$. As in the previous chapter, double edges indicate
        completeness, while no edges indicate anticompleteness.}
    \label{fig:monopolar_partition_general_c5}
\end{figure}

\begin{corollary}
    A connected monopolar graph $(P_5,\text{house})$-free with an induced copy
    of $C_5$ has the structure of \Cref{fig:monopolar_partition_general_c5}.
\end{corollary}

Finally, we are left to discuss the time complexity of the procedures described
by the lemmas and theorem of the chapter. Observe that all instructions in this
case rely only on the exact neighborhoods: checking whether they are nonempty,
they induce clusters or complete graphs, or they are complete or anticomplete to
other exact neighborhoods. Recall that these checks can be done in linear time.
When $N_\varnothing$ is nonempty an execution of BFS returns an induced copy of
$P_5$ by getting a path of minimal length from $N_\varnothing$ to the induced
copy of $C_5$. Henceforth, the next corollary takes place.

\begin{corollary}
    \label{cor:algorithm_c5}
    There is a linear-time certifying algorithm that decides whether a graph
    with an induced copy of $C_5$ is monopolar, $P_5$-free and house-free,
    provided that an induced copy of $C_5$ is given as an input. In particular,
    deciding monopolarity in graphs with an induced copy of $C_5$ is linear-time
    solvable on $(P_5,\text{house})$-free graphs, once more provided that an
    induced copy of $C_5$ is given as an input.
\end{corollary}