...
induce clusters or complete graphs, or are complete or anticomplete to other
exact neighborhoods. Recall that these checks can be done in linear time. As for
$N_\varnothing$, it being nonempty returns an induced copy of $P_5$ by executing
a BFS to get a path of minimal length from $N_\varnothing$ to the induced copy
of $C_5$. Henceforth, the next corollary takes place.

\begin{corollary}
    \label{cor:algorithm_c5}
    There is a linear-time certyfing algorithm that decides whether a graph with
    an induced copy of $C_5$ is monopolar, $P_5$-free and house-free, provided
    that an induced copy of $C_5$ is given as an input. In particular, deciding
    monopolarity in graphs with an induced copy of $C_5$ is linear-time solvable
    on $(P_5,\text{house})$-free graphs, once more provided that an induced copy
    of $C_5$ is given as an input.
\end{corollary}