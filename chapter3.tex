...
\section{Structure and time complexity of the algorithm}
...
Finally, we are left to argue discuss the time-complexity of the procedures
described by the lemmas and theorem of the chapter. Observe that all
instructions in this case rely only on the exact neighborhoods: checking whether
they are nonempty, induce clusters or complete graphs, or are complete or
anticomplete to other exact neighborhoods. Recall that these checks can be done
in linear time. As for $N_\varnothing$, it being nonempty returns an induced
copy of $P_5$ by executing a BFS to get a path of minimal length from
$N_\varnothing$ to the induced copy of $C_5$. Henceforth, the next corollary
takes place.
